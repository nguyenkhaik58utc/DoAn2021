...
namespace iDAS.Services
{
    public class FileSV : BaseService
    {
        ...
        {
            var name = fileDA.GetQuery(i => i.ID == Id)
        ...
        public bool Delete(System.Guid id)
        {
            ...
            var File = dbo.AttachmentFiles.FirstOrDefault(i => i.ID == id);
            dbo.AttachmentFiles.Remove(File);
            dbo.SaveChanges();
            return true;
        }
        public Guid Insert(System.Web.HttpPostedFileBase file, int userID)
        {
            if (file != null)
            {
                var fileInsert = new AttachmentFile()
                 {
                     ID = Guid.NewGuid(),
                     Name = file.FileName,
        ...
        {
            var lsFileInsert = new List<AttachmentFile>();
            foreach (var file in files)
            {
                if (file != null)
                    lsFileInsert.Add(new AttachmentFile()
                                {
                                    ID = Guid.NewGuid(),
                                    Name = file.FileName,
                                    Extension = string.IsNullOrWhiteSpace(file.FileName) ?
        ...
        public void Delete<TEntity>(int paramID) where TEntity : class
        {
            ...
            var paramName = typeof(TEntity).GetProperties()
                    ...
                    .Where(i => !i.Name.Contains("AttachmentFileID"))
            ...
            var attachmentFileIDs = t.Select(i => i.GetType().GetProperty("AttachmentFileID").GetValue(i)).ToList();
        ...
        private void insert<TEntity>(List<Guid> fileIDs, int paramID) where TEntity : class
        {
            ...
            {
                dbo.Entry(obj).Property("AttachmentFileID").CurrentValue = fileID;
        ...
        private void delete<TEntity>(List<Guid> fileIDs, int paramID) where TEntity : class
        {
            ...
            {
                var t = table
                        .Where(i => i.GetType().GetProperty("AttachmentFileID").GetValue(i).ToString().Contains(fileID.ToString()))
        ...
        public string ConvertDocxToHtml(byte[] fileData)
        {
            ...
            {
                // HungNM. Fix error read file. 20201014.
