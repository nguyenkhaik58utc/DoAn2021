...
namespace iDAS.Services
{
    ...
    public class RiskLegalSV
    {
        private iDASBusinessEntities dbo = new RiskLegalDA().Repository;
        public List<RiskLegalItem> GetByDepartment(ModelFilter filter, int departmentId)
        {
            var riskItem = dbo.RiskLegals
                    .Where(t => t.Risk.HumanDepartmentID == departmentId)
                    .Select(i => new { risk = i.Risk, legal = i.DepartmentLegal }).Distinct()
                    .Select(item => new RiskLegalItem()
                    {
                        ID = item.risk.ID,
                        DepartmentLegalName = item.legal.Name,
        ...
        public void Insert(RiskItem item, int userID)
        {
            ...
            dbo.RiskLegals.Add(new RiskLegal()
            {
                RiskID = insertItem.ID,
                DepartmentLegalID = item.LegalID,
                CreateAt = DateTime.Now,
                CreateBy = userID
            });
            dbo.SaveChanges();
        ...
        public void Update(RiskItem item, int userID)
        {
            ...
            if (!risk.IsSend)
            {
                ...
                var riskLegal = risk.RiskLegals.FirstOrDefault();
                if (riskLegal != dbo.RiskLegals.Where(i => i.ID == item.LegalID).FirstOrDefault())
                {
                    if (riskLegal != null)
                    {
                        dbo.RiskLegals.Remove(riskLegal);
                    }
                    risk.RiskLegals.Add(new RiskLegal()
                    {
                        RiskID = item.ID,
                        DepartmentLegalID = item.LegalID,
                        CreateAt = DateTime.Now,
        ...
        {
            dbo.RiskLegals.RemoveRange(dbo.RiskLegals.Where(t => t.RiskID == riskId));
