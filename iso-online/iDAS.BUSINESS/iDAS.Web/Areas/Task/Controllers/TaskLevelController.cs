...
{
    [BaseSystem(Name = "Danh mục mức độ công việc", IsActive = true, Icon = "PageGear", IsShow = true, Position = 3)]
    [SystemAuthorize(CheckAuthorize = false)]
    public class TaskLevelController : BaseController
    {
        //
        // GET: /Task/TaskLevel/
        private TaskLevelSV taskLevelSV = new TaskLevelSV();
        private readonly string storeLevel = "stTaskLevel";
        ...
        {
            List<TaskLevelItem> lst;
            int total;
            lst = taskLevelSV.GetAll(parameters.Page, parameters.Limit, out total);
            return this.Store(new Paging<TaskLevelItem>(lst, total));
        }
        ...
        public ActionResult Insert(string name, string color, bool inused, string des)
        {
            try
            {
                ...
                {
                    taskLevelSV.Insert(name, color, inused, des, User.ID);
        ...
        public ActionResult Update(int id, string name, string color, bool inused, string des)
        {
            try
            {
                ...
                {
                    taskLevelSV.Update(id, name, des, color, inused, User.ID);
                    X.GetCmp<Store>(storeLevel).Reload();
