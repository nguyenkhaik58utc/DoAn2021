...
namespace iDAS.Web.Areas.Document.Controllers
{
    ...
    [BaseSystem(Name = "Tài liệu nội bộ", Icon = "Page", IsActive = true, IsShow = true, Position = 1, Parent = GroupDocumentsController.Code)]
    public class DocumentController : BaseController
    {
        ...
        public ActionResult LoadData(StoreRequestParameters parameters, int folderID = 0)
        {
            ...
            //var lst = documentSV.GetAllTypeInByDepartmentID(filter, groupId, focusId);
            //return this.Store(lst, filter.PageTotal);

            var resp = api.GetData(parameters.Page, parameters.Limit, folderID, User.ID, User.EmployeeID, false);
        ...
        public ActionResult Insert(DocumentItem obj, [Bind(Prefix = "StorageFormID")] string[] borders, bool IsSendApproval = false)
        {
            ...
            if (id > 0)
            {
                if (IsSendApproval)
                {
                    ...
                    notify.Notify(this.ModuleCode, "Có một tài liệu yêu cầu phê duyệt", obj.Name, obj.Approval.ID, User, Common.Document, "DocumentID:" + id.ToString());
                }
        ...
        public ActionResult Update(int id)
        {
            ...
            obj.QuickDownloadLink = "";
            if(obj.AttachmentFile != null && obj.AttachmentFile.Files != null && obj.AttachmentFile.Files.Count > 0)
            {
                for(int i=0; i< obj.AttachmentFile.Files.Count; i++)
                {
                    obj.QuickDownloadLink += string.Format("  {0}/Generic/File/D?f={1}", Request.Url.Authority, obj.AttachmentFile.Files[i]);
                }
                obj.QuickDownloadLink = obj.QuickDownloadLink.Trim();
            }
        ...
        public ActionResult Update(DocumentItem obj, [Bind(Prefix = "StorageFormID")] string[] borders, bool IsSendApproval = false)
        {
            ...
            if (update(obj))
            {
                ...
                {
                    NotifyController notify = new NotifyController();
                    notify.Notify(this.ModuleCode, "Có một tài liệu yêu cầu phê duyệt", obj.Name, obj.Approval.ID, User, Common.Document, "DocumentID:" + obj.ID.ToString());
                }
        ...
        public ActionResult Approve(DocumentItem obj)
        {
            ...
            {
                NotifyController notify = new NotifyController();
                notify.Notify(this.ModuleCode, "Có một tài liệu đã được phê duyệt", obj.Name, obj.EmployeesCreateID, User, Common.Document, "DocumentID:" + obj.ID.ToString());
                Ultility.CreateNotification(message: RequestMessage.UpdateSuccess);
                X.GetCmp<Store>("StDocument").Reload();
        ...
        public ActionResult InsertDocument(DocumentItem obj, [Bind(Prefix = "StorageFormID")] string[] borders, bool IsSendApproval = false)
        {
            try
            {
                ...
                {
                    NotifyController notify = new NotifyController();
                    notify.Notify(this.ModuleCode, "Có một tài liệu nội bộ sửa đổi yêu cầu phê duyệt", obj.Name, obj.Approval.ID, User, Common.Document, "DocumentID:" + id.ToString());
                }
                X.GetCmp<Store>("StDocument").Reload();
                X.GetCmp<Window>("winAddDocument").Close();
                Ultility.CreateNotification(message: RequestMessage.CreateSuccess);
        ...
        [SystemAuthorize(CheckAuthorize = true)]
        [HttpGet]
        public ActionResult Permission(int id, bool isPublic)
        {
            // var obj = documentSV.GetByID(id);
            ...
            return new Ext.Net.MVC.PartialViewResult { ViewName = "Permission", Model = objN, ViewData = ViewData };
        }
        DocumentEmployeeAPI documentEmployeeAPI = new DocumentEmployeeAPI();
        DocumentDepartmentAPI documentDepartmentAPI = new DocumentDepartmentAPI();
        public ActionResult GetEmployeePerformedDocument(int documentId = 0)
        {
            ...
            {
                data = documentEmployeeAPI.GetListByDocumentID(documentId).Result.Data;
            ...
            var obj = documentSV.GetByID(documentId);
            foreach (var item1 in data)
            {
                if (obj.AllUserAccess == true)
                {
                    item1.CheckedShow = false;
                }
            }
            return this.Store(data);
        }
        private DepartmentSV departmentSV = new DepartmentSV();
        private List<DocumentDepartmentDTO> lstPermission = new List<DocumentDepartmentDTO>();
        public ActionResult LoadDocumentDepartment(string node, bool viewDeactive = false, bool viewCancel = false, bool viewMerge = false, bool viewDelete = false, string departmentIds = "", int documentId = 0)
        {
            try
            {
                var obj = documentSV.GetByID(documentId);
                List<DocumentDepartmentDTO> lst = new List<DocumentDepartmentDTO>();
                lst = documentDepartmentAPI.GetListByDocumentID(documentId).Result.Data;
                foreach (var item1 in lst)
                {
                    ...
                    lstPermission.Add(item1);
                }

                Session["LstPermission"] = lstPermission;
                ...
                DocumentDepartmentViewModel item = new DocumentDepartmentViewModel();
                foreach (var department in departments)
                {
                    foreach (var index in lst)
                    {
                        if (department.ID == index.DepartmentID && index.IsUpdate == true)
                        {
                            item.IsUpdate = true;
                            break;
                        }
                        else item.IsUpdate = false;

                    }
                    foreach (var index in lst)
                    {
                        if (department.ID == index.DepartmentID && index.IsShow == true)
                        {
                            item.IsShow = true;
                            break;
                        }
                    ...
                    item.IsDelete = department.IsDelete;
                    item.IsMerge = department.IsMerge;
                    item.IsCancel = department.IsCancel;
                    item.IsSelected = department.IsSelected;
                    if (obj.AllUserAccess == true)
                    {
                        item.IsShow = false;
                    }
                    item.IsParent = department.IsParent;
                    //gán quyền
                    for (int i = 0; i < lstPermission.Count; i++)
                    {
                        if (lstPermission[i].DepartmentID == department.ID)
                        {
                            item.IsAccess = true;
                            break;
                        }
                    ...
                    nodes.Add(createNodeDepartment(item));
                }
        ...
        private Node createNodeDepartment(DocumentDepartmentViewModel department)
        {
            Node node = new Node();
            ...
            if (department.IsUpdate == true)
            {
                css = "DepartmentUpdate";
            }
            if (department.IsShow == true)
            {
                css = "DepartmentShow";
            }
            ...
            node.CustomAttributes.Add(new ConfigItem { Name = "IsAccess", Value = JSON.Serialize(department.IsAccess), Mode = ParameterMode.Value });
            node.CustomAttributes.Add(new ConfigItem { Name = "IsUpdate", Value = JSON.Serialize(department.IsUpdate), Mode = ParameterMode.Value });
            node.CustomAttributes.Add(new ConfigItem { Name = "IsShow", Value = JSON.Serialize(department.IsShow), Mode = ParameterMode.Value });
            node.Leaf = !department.IsParent;
            return node;
        }

        public ActionResult UpdateDepartmentPermission(string dataPerform, bool checkAll, string DocumentId)
        {
            var result = false;
            try
            {
                UpdatePulicPermission(checkAll, DocumentId);
                //if (dataPerform != null && dataPerform.Length > 2)
                //{
                var dataUpdated = JSON.Deserialize<Dictionary<string, object>>(dataPerform ?? string.Empty)["Updated"];
                var lstUpdate = ((Newtonsoft.Json.Linq.JArray)dataUpdated);
                var lstAdd = new List<DocumentDepartmentDTO>();
                //var lstDepartmentId = new List<int>();
                for (int j = 0; j < lstUpdate.Count; j++)
                {
                    lstAdd.Add(new DocumentDepartmentDTO(int.Parse(lstUpdate[j]["id"].ToString()), Int32.Parse(DocumentId), bool.Parse(lstUpdate[j]["IsShow"].ToString()), bool.Parse(lstUpdate[j]["IsUpdate"].ToString())));
                }
                if (Session["LstPermission"] != null)
                {
                    lstPermission = (List<DocumentDepartmentDTO>)Session["LstPermission"];
                }
                foreach (var index in lstPermission)
                {
                    foreach (var item in lstAdd)
                    {
                        if (index.DepartmentID == item.DepartmentID)
                        {
                            index.ID = item.ID;
                            index.DocumentID = item.DocumentID;
                            index.DepartmentID = item.DepartmentID;
                            index.IsShow = item.IsShow;
                            index.IsUpdate = item.IsUpdate;
                        }
                    }
                }
                lstAdd.AddRange(lstPermission);
                DocumentDepartmentSaveRequest request = new DocumentDepartmentSaveRequest();
                request.DepartmentIDList = lstAdd;
                request.DocumentID = int.Parse(DocumentId);
                documentDepartmentAPI.SavePermission(request);
                Session["LstPermission"] = lstAdd;
                Ultility.CreateNotification(message: RequestMessage.UpdateSuccess);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult UpdatePulicPermission(bool dataPerform, string DocumentId)
        {
            var result = false;
            try
            {
                DocumentPublicSaveRequest request = new DocumentPublicSaveRequest();
                request.DocumentID = int.Parse(DocumentId);
                request.IsPublic = dataPerform;
                string res = api.SavePublicPermission(request);
            }
            catch (Exception ex)
            {
                Ultility.CreateNotification(message: RequestMessage.UpdateError);
        ...
        public ActionResult UpdatePermission(string dataPerform,bool checkAll, string DocumentId)
        {
            var result = false;
            try
            {
                ...
                var dataUpdated = JSON.Deserialize<Dictionary<string, object>>(dataPerform ?? string.Empty)["Updated"];
                var lstUpdate = ((Newtonsoft.Json.Linq.JArray)dataUpdated);
                var lstAdd = new List<DocumentEmployeeSaveDTO>();
                List<DocumentEmployeeViewModel> data = new List<DocumentEmployeeViewModel>();
                if (Session["EmpDocument"] != null)
                {
                    data = (List<DocumentEmployeeViewModel>)Session["EmpDocument"];
                }
                var lstEmployee = new List<DocumentEmployeeSaveDTO>();
                foreach (var item in data)
                {
                    lstEmployee.Add(new DocumentEmployeeSaveDTO(item.ID, item.CheckedShow, item.CheckedEdit));
                }

                for (int j = 0; j < lstUpdate.Count; j++)
                {
                    lstAdd.Add(new DocumentEmployeeSaveDTO(int.Parse(lstUpdate[j]["ID"].ToString()), bool.Parse(lstUpdate[j]["CheckedShow"].ToString()), bool.Parse(lstUpdate[j]["CheckedEdit"].ToString())));
                }
                foreach (var index in lstEmployee)
                {
                    foreach (var item in lstAdd)
                    {
                        if (index.EmployeeID == item.EmployeeID)
                        {
                            index.EmployeeID = item.EmployeeID;
                            index.CheckedShow = item.CheckedShow;
                            index.CheckedEdit = item.CheckedEdit;
                        }
                    }
                }
                ...
                DocumentEmployeeSaveRequest request = new DocumentEmployeeSaveRequest();
                request.EmployeeList = lstEmployee;
                request.DocumentID = int.Parse(DocumentId);
                documentEmployeeAPI.SavePermission(request);
                Ultility.CreateNotification(message: RequestMessage.UpdateSuccess);
            }
            catch (Exception ex)
            {
        ...
        //public ActionResult GetDepartmentPerformedDocument(int documentId = 0)
        //{
        //    List<DocumentDepartmentViewModel> data = new List<DocumentDepartmentViewModel>();
        //    //(List<EmployeeBO>)Session["EmpTaskSchedule"];
        //    if (Session["DepartmentDocument"] != null)
        //    {
        //        data = (List<DocumentDepartmentViewModel>)Session["DepartmentDocument"];
        //    }
        //    else if (documentId != 0)
        //    {
        //        data = documentDepartmentAPI.GetListByDocumentID(documentId).Result.Data;
        //        Session["DepartmentDocument"] = data;
        //    }
        //    return this.Store(data);
        //}
        ...
        public ActionResult LoadTreeFolder(string node)
        {
            try
            {
                ...
                var allFolder = api.GetFolder(false).Result;
